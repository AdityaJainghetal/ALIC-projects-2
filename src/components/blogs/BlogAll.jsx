...
//     try {
//       const response = await fetch('https://backend-alic-3.onrender.com/blog/display');
...
export const BlogAll = () => {
  ...
  const displayBlogs = filteredBlogs.length > 0 ? filteredBlogs : 
                      allBlogs.length > 0 ? allBlogs : fallbackBlogPosts;
        ...
        <div className="row">
          ...
          <div className="col-lg-8">
            {selectedCategoryId && filteredBlogs.length === 0 ? (
              <NoBlogsMessage 
                clearFilter={() => setSelectedCategoryId(null)}
              />
            ) : (
