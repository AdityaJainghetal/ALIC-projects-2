...
const DetailSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://backend-alic-4.onrender.com/whatsnew/${id}`);
        if (!response.ok) {
      ...
      <section className="py-5 bg-light">
        <div className="container">
          <div className="row align-items-center">
            ...
            {/* Right Side Content */}
            <div className="col-md-6">
              <h2 className="mb-3 text-danger" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(whatsNew.Coursename) }} />
              ...
              >
                ...
                {whatsNew.PDFbrochure ? 'Download Brochure' : 'Brochure Not Available'}
              </button>
              ...
              {whatsNew.additionalDetails && (
                <div className="mt-4">
                  ...
                  <p dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(whatsNew.additionalDetails) }} />
                </div>
              )}
            </div>
          </div>
