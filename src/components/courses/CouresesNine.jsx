...
//     try {
//       setLoading(true);
//       const res = await axios.get(`https://backend-alic-1.onrender.com/api/coursess/${id}`);
//       const mainCourse = res.data;
//       setProduct(mainCourse);

//       const relatedRes = await axios.get(
//         `https://backend-alic-1.onrender.com/api/courses?category=${mainCourse.category._id}`
//       );
  ...
  useEffect(() => {
    const fetchCourseAndSameCategory = async () => {
      ...
      try {
        ...
        const res = await axios.get(`https://backend-alic-1.onrender.com/api/courses/${id}`);
        const main = res.data;
        console.log(main)
        setMainCourse(main);
        ...
        const sameCategoryRes = await axios.get(
          `https://backend-alic-1.onrender.com/api/courses/category/${main.category._id}`
        );
          ...
          {sameCategoryCourses.length > 0 ? (
            sameCategoryCourses.map((course) => (
              <div key={course._id} className="col-lg-4 col-md-6">
                <div className="td_card td_style_3 d-block td_radius_10">
                  <span className="td_card_label td_accent_bg td_white_color">New</span>
                  <Link to={`/courses/${course._id}`} className="td_card_thumb">
                    <img src={course.images?.[0] || "default-course-image.jpg"} alt="Course" />
                  </Link>
                  <div className="td_card_info td_white_bg">
                    <div className="td_card_info_in">
                      <ul className="td_card_meta td_mp_0 td_fs_18 td_medium td_heading_color">
                      ...
                      <div className="td_card_btn">
                        <Link to="/cart" className="td_btn td_style_1 td_radius_10 td_medium">
                          <span className="td_btn_in td_white_color td_accent_bg">Enroll Now</span>
                        </Link>
                      </div>
                    </div>
                  </div>
