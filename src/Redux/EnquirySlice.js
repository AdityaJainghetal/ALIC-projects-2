...
export const submitEnquiry = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      const response = await axios.post("https://backend-alic-1.onrender.com/enquiry/add", formData);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(submitEnquiry.rejected, (state, action) => {
        state.isLoading = false;
        state.errorMessage = action.payload || "Failed to send message. Please try again later.";
      });
  },
});
